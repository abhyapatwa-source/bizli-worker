...
html{scroll-behavior:smooth;font-size:calc(clamp(12px,0.92vw,17px) * var(--ui-scale,1))}
...
.summary-row{
  display:flex;gap:10px;padding:12px 16px;margin-bottom:12px;align-items:center;
  background:rgba(0,212,255,.04);border:1px solid rgba(0,212,255,.1);border-radius:10px;
}
.sboxes{display:flex;gap:10px;flex:1;flex-wrap:wrap;min-width:0}
.summary-toggle{flex-shrink:0;align-self:center;width:26px;height:26px;border-radius:7px;border:1px solid rgba(0,212,255,.2);background:rgba(0,212,255,.05);color:var(--blue);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s,transform .3s ease}
.summary-toggle:hover{background:rgba(0,212,255,.12)}
.summary-toggle svg{width:15px;height:15px;stroke-width:2}
.summary-row.collapsed{padding-top:6px;padding-bottom:6px}
.summary-row.collapsed .sboxes{display:none}
.summary-row.collapsed .summary-toggle{transform:rotate(180deg)}
.sbox{flex:1;min-width:70px;text-align:center;position:relative;padding-bottom:10px}
.sbox::after{content:"";position:absolute;bottom:0;left:10%;right:10%;height:1.5px;background:linear-gradient(90deg,transparent,var(--blue),transparent);border-radius:2px}
.sico{height:16px;display:flex;align-items:center;justify-content:center;margin-bottom:3px;color:var(--blue)}
.sico svg{width:13px;height:13px;stroke-width:1.5;stroke:currentColor;fill:none}
.snum{font-size:1.55rem;font-weight:700;color:var(--blue);font-family:"Courier New",monospace;min-width:1ch}
.slbl{font-size:.74rem;color:var(--muted);letter-spacing:.12em;margin-top:2px;margin-bottom:3px}
.grid{
...
.ptitle{
  font-size:.82rem;letter-spacing:.2em;color:var(--blue);margin-bottom:11px;
...
#orb-section{
  ...
  padding:6px 2px;min-height:340px;position:relative;
}
...
#cat-wrap{position:relative;width:97%;aspect-ratio:1130/1392;margin:0 auto;flex-shrink:0}
#cat-holo{
...
#cat-holo.red{box-shadow:0 0 34px rgba(0,212,255,.20),inset 0 0 24px rgba(0,212,255,.10)}
...
/* Blink — briefly dims her glowing eyes with her own holographic-blue tone (not black) */
.eyelid{
  position:absolute;pointer-events:none;opacity:0;z-index:2;
  border-radius:50%;filter:blur(1.4px);
  background:radial-gradient(ellipse at center,rgba(26,70,120,.96) 44%,rgba(26,70,120,0) 100%);
  animation:eyeblink 7.2s ease-in-out infinite;
}
.eyelid.el{left:35%;top:23.5%;width:8.5%;height:5%}
.eyelid.er{left:49.5%;top:23.5%;width:8.5%;height:5%}
...
@keyframes tailswish{0%,100%{transform:rotate(0deg)}30%{transform:rotate(-10deg)}70%{transform:rotate(10deg)}}
#ear-l-grp{transform-box:fill-box;transform-origin:50% 100%}
#ear-r-grp{transform-box:fill-box;transform-origin:50% 100%}
#ear-l-grp.twitching{animation:eartwitch .55s ease-in-out forwards}
#ear-r-grp.twitching{animation:eartwitch .55s ease-in-out forwards}
@keyframes eartwitch{0%,100%{transform:rotate(0deg)}40%{transform:rotate(-10deg)}75%{transform:rotate(5deg)}}
#eye-l-grp,#eye-r-grp{transform-box:fill-box;transform-origin:center;transition:transform .11s ease}
#pupil-left,#pupil-right,#shine-left,#shine-right{transition:transform .13s ease-out}
.cat-scanline{
...
.r1{width:210px;height:210px;animation:rspin 7s linear infinite;border-color:rgba(0,212,255,.4)}
.r2{width:252px;height:252px;animation:rspin 11s linear infinite reverse;border-color:rgba(61,219,217,.28)}
.r3{width:292px;height:292px;animation:rspin 17s linear infinite;border-color:rgba(0,212,255,.16);border-style:dashed}
.r4{width:326px;height:326px;animation:rspin 24s linear infinite reverse;border-color:rgba(61,219,217,.1);border-style:dotted}
.r5{width:358px;height:358px;animation:rspin 33s linear infinite;border-color:rgba(0,212,255,.07);border-width:2px}
@keyframes rspin{
...
#drive-section{grid-column:3;grid-row:1/3;display:flex;flex-direction:column}
...
.ditem{
  ...
  background:rgba(0,212,255,.03);border:1px solid rgba(0,212,255,.08);font-size:.8rem;
}
...
.ticon-lbl{font-size:.62rem;letter-spacing:.05em;color:var(--green);text-align:center;opacity:.85}
...
.met-lbl{font-size:.74rem;letter-spacing:.12em;color:var(--muted)}
.met-val{font-size:.82rem;font-weight:700;color:var(--cyan)}
...
.vrow{
  ...
  padding:6px 0;border-bottom:1px solid rgba(0,212,255,.06);font-size:.8rem;
}
.vrow:last-child{border-bottom:none}
.vk{color:var(--muted);font-size:.78rem;letter-spacing:.08em}
...
.lab-title{font-size:.88rem;letter-spacing:.28em;color:var(--blue);font-weight:700}
#lab-snd-btn{background:none;border:1px solid rgba(0,212,255,.15);border-radius:4px;cursor:pointer;font-size:.66rem;letter-spacing:.08em;color:var(--muted);padding:2px 6px;transition:color .15s,border-color .15s;margin-left:auto;margin-right:6px;white-space:nowrap}
...
.lbmsg{max-width:85%;padding:9px 12px;border-radius:12px;font-size:.88rem;line-height:1.55;word-break:break-word;white-space:pre-wrap}
...
.lbsys{align-self:center;color:var(--muted);font-size:.76rem;font-style:italic;max-width:100%;text-align:center;padding:4px 0}
#lab-proc{
  padding:8px 16px;font-size:.76rem;color:var(--cyan);letter-spacing:.1em;
...
#lab-ta{
  ...
  border-radius:8px;padding:8px 10px;font-family:"Courier New",monospace;font-size:.85rem;
...
#lab-toggle{
  display:none;position:fixed;right:14px;
  bottom:calc(14px + env(safe-area-inset-bottom,0px));
  height:36px;padding:0 15px;border-radius:20px;
  background:rgba(8,14,26,.78);border:1px solid rgba(0,212,255,.45);
...
#lab-toggle::before{content:"\\25C9";font-size:.7rem;opacity:.9}
#leftnav{
...
/* AGENT nav item + Lab close button: phone-only */
.lnav-agent{display:none}
#lab-close{display:none;flex-shrink:0;width:26px;height:26px;margin-left:4px;border-radius:6px;border:1px solid rgba(0,212,255,.2);background:rgba(0,212,255,.05);color:var(--blue);cursor:pointer;align-items:center;justify-content:center;font-size:.82rem;line-height:1}
#lab-close:hover{background:rgba(0,212,255,.12)}
@media(max-width:800px){
  #app{margin-left:0!important;margin-right:0!important;padding-bottom:16px}
  ...
  #lab-close{display:flex}
  .lnav-agent{display:flex}
...
#lab-quota-text{font-size:.75rem;color:var(--muted);margin-bottom:3px;letter-spacing:.04em}
...
.plname{font-size:.65rem;letter-spacing:.09em;color:var(--text)}
.plsub{font-size:.62rem;color:var(--cyan);margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;opacity:.75}
...
#app.tab-settings #orb-section,#app.tab-settings #brain-section,#app.tab-settings #drive-section,#app.tab-settings #err-section,#app.tab-settings #users-section,#app.tab-settings #tools-section,#app.tab-settings #vitals-section,#app.tab-settings #brains-section{display:none}
.set-block{margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid rgba(0,212,255,.07)}
.set-block:last-of-type{border-bottom:none;margin-bottom:6px}
.set-hdr{display:flex;align-items:center;gap:8px;font-size:.82rem;letter-spacing:.14em;color:var(--blue);margin-bottom:6px}
.set-hdr svg{width:15px;height:15px;stroke-width:1.6}
.set-desc{font-size:.7rem;color:var(--muted);line-height:1.55;margin-bottom:12px}
.mus-now{font-size:.74rem;color:var(--cyan);letter-spacing:.06em;margin-bottom:12px;min-height:17px}
.mus-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px}
.mus-chip{padding:10px 6px;border-radius:8px;border:1px solid rgba(0,212,255,.18);background:rgba(0,212,255,.04);color:var(--text);font-size:.72rem;letter-spacing:.03em;text-align:center;cursor:pointer;transition:all .2s;font-family:inherit}
.mus-chip:hover{border-color:rgba(0,212,255,.5);background:rgba(0,212,255,.1);transform:translateY(-1px)}
.mus-chip.active{border-color:var(--cyan);background:rgba(61,219,217,.15);color:var(--cyan);box-shadow:0 0 12px rgba(61,219,217,.28)}
.set-row{display:flex;align-items:center;gap:14px;margin-bottom:10px}
.set-lbl{font-size:.7rem;letter-spacing:.12em;color:var(--muted);min-width:66px}
.set-slider{flex:1;accent-color:var(--blue);height:4px;cursor:pointer}
.set-val{font-size:.7rem;color:var(--blue);min-width:42px;text-align:right;font-family:"Courier New",monospace}
.set-select{flex:1;background:#0d1a30;border:1px solid rgba(0,212,255,.3);color:var(--text);padding:7px 10px;border-radius:7px;font-family:inherit;font-size:.74rem;letter-spacing:.04em;cursor:pointer;outline:none}
.set-select:focus{border-color:var(--blue);box-shadow:0 0 8px rgba(0,212,255,.25)}
.set-btn{background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.35);color:var(--red);padding:7px 18px;border-radius:7px;font-family:inherit;font-size:.72rem;letter-spacing:.1em;cursor:pointer;transition:all .2s}
.set-btn:hover{background:rgba(239,68,68,.18)}
.set-check{display:flex;align-items:center;gap:9px;font-size:.76rem;color:var(--text);cursor:pointer;letter-spacing:.03em;margin-bottom:8px}
.set-check input{width:15px;height:15px;accent-color:var(--blue);cursor:pointer;flex-shrink:0}
.set-toast{display:none;margin-top:8px;font-size:.7rem;color:var(--green);letter-spacing:.08em;text-align:center}
...
/* LAB AGENT right panel — Vision AI section */
.lab-subtitle{font-size:.68rem;letter-spacing:.1em;color:var(--muted);margin-top:2px;display:flex;align-items:center;gap:5px}
.lab-dot-inline{display:inline-block;width:5px;height:5px;border-radius:50%;background:var(--green);box-shadow:0 0 5px var(--green);animation:lpulse 1.4s infinite;vertical-align:middle}
#lab-va{padding:10px 14px 10px;border-bottom:1px solid rgba(0,212,255,.08);flex-shrink:0}
.lva-title{font-size:.74rem;letter-spacing:.18em;color:var(--blue);margin-bottom:4px}
.lva-desc{font-size:.78rem;color:var(--muted);line-height:1.55;margin-bottom:8px;text-align:center}
#lva-orb{width:108px;height:144px;display:none;align-items:center;justify-content:center;position:relative}
.vai-core{
  width:38px;height:38px;border-radius:50%;position:absolute;z-index:2;
  background:radial-gradient(circle at 38% 38%,rgba(0,212,255,.95),rgba(0,40,80,.7));
  box-shadow:0 0 18px rgba(0,212,255,.65),0 0 36px rgba(0,212,255,.28),inset 0 0 12px rgba(0,212,255,.35);
  animation:vaicore 3s ease-in-out infinite;
}
@keyframes vaicore{0%,100%{box-shadow:0 0 18px rgba(0,212,255,.65),0 0 36px rgba(0,212,255,.28)}50%{box-shadow:0 0 28px rgba(0,212,255,.9),0 0 54px rgba(0,212,255,.45)}}
.vai-ring{position:absolute;border-radius:50%;border:1px solid rgba(0,212,255,.3)}
.vai-r1{width:60px;height:60px;animation:vaispin 4s linear infinite;border-color:rgba(0,212,255,.45)}
.vai-r2{width:82px;height:82px;animation:vaispin 7s linear infinite reverse;border-style:dashed;border-color:rgba(61,219,217,.25)}
.vai-r3{width:104px;height:104px;animation:vaispin 13s linear infinite;border-color:rgba(0,212,255,.12)}
@keyframes vaispin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}
.vai-node{position:absolute;width:5px;height:5px;border-radius:50%;background:var(--cyan);box-shadow:0 0 5px var(--cyan);top:calc(50% - 2.5px);left:calc(50% - 2.5px)}
.vai-n1{animation:vain1 4s linear infinite}
.vai-n2{animation:vain2 7s linear infinite reverse .8s;background:var(--blue);box-shadow:0 0 5px var(--blue)}
.vai-n3{animation:vain3 12s linear infinite 1.5s;opacity:.7}
@keyframes vain1{from{transform:rotate(0deg) translateX(30px)}to{transform:rotate(360deg) translateX(30px)}}
@keyframes vain2{from{transform:rotate(0deg) translateX(41px)}to{transform:rotate(360deg) translateX(41px)}}
@keyframes vain3{from{transform:rotate(0deg) translateX(52px)}to{transform:rotate(360deg) translateX(52px)}}
#lab-quick{display:flex;flex-direction:column;gap:4px}
.lqa{font-size:.79rem;color:var(--blue);cursor:pointer;padding:4px 0 4px 9px;letter-spacing:.04em;border-left:2px solid rgba(0,212,255,.28);transition:border-color .15s,color .15s,background .15s}
.lqa:hover{color:var(--cyan);border-left-color:var(--cyan);background:rgba(0,212,255,.04)}
/* System Metrics 2x2 grid */
.met-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px}
.met-cell{background:rgba(0,212,255,.03);border:1px solid rgba(0,212,255,.08);border-radius:8px;padding:9px 6px;text-align:center;transition:border-color .35s}
.met-cell:hover{border-color:rgba(0,212,255,.22)}
.met-cell-lbl{font-size:.55rem;letter-spacing:.14em;color:var(--muted);margin-bottom:5px}
.met-cell-val{font-size:1.1rem;font-weight:700;letter-spacing:.04em}
.met-ok .met-cell-val{color:var(--green)}
.met-warn .met-cell-val{color:var(--amber)}
.met-crit .met-cell-val{color:var(--red)}
.met-blue .met-cell-val{color:var(--cyan)}
/* Page footer */
...
.t-lab-sub{font-size:.58rem;letter-spacing:.06em;color:var(--muted);display:flex;align-items:center;gap:4px;justify-content:flex-end}
.t-lab-arrows{color:var(--muted);font-size:.65rem;letter-spacing:.06em;flex-shrink:0}
/* Right panel robot image + row layout */
.lva-row{display:flex;flex-direction:column-reverse;gap:8px;align-items:center;margin-bottom:8px}
.lva-text-col{flex:1;min-width:0}
.lva-img-col{flex-shrink:0;width:100%;display:flex;align-items:center;justify-content:center}
#vai-robot-img{width:170px;height:auto;max-height:210px;object-fit:contain;mix-blend-mode:screen;-webkit-mask-image:radial-gradient(ellipse 78% 88% at 50% 46%,#000 50%,transparent 100%);mask-image:radial-gradient(ellipse 78% 88% at 50% 46%,#000 50%,transparent 100%);filter:drop-shadow(0 0 16px rgba(0,212,255,.5)) brightness(1.08) saturate(1.2)}
...
.lc-box{flex:1 1 auto;min-height:130px;margin-top:10px;position:relative;border-radius:6px;overflow:hidden;border:1px solid rgba(34,197,94,.12);background:#020604}
.lc-tag{position:absolute;top:5px;right:8px;z-index:2;font-size:.52rem;letter-spacing:.14em;color:rgba(34,197,94,.5);pointer-events:none;font-family:"Courier New",monospace}
.lc-stream{position:absolute;inset:0;overflow:hidden;padding:9px 11px;font-family:"Courier New",monospace;font-size:.63rem;line-height:1.55;color:rgba(34,197,94,.82);white-space:pre-wrap;word-break:break-all;display:flex;flex-direction:column;justify-content:flex-end}
.lc-stream .lcs-cmt{color:rgba(34,197,94,.4)}
.lc-stream .lcs-err{color:#ff6b6b}
.lc-stream .lcs-key{color:#3ddbd9}
.lc-stream .lcs-kw{color:#5fb0ff}
.lcs-cur{display:inline-block;animation:lcblink 1s steps(1) infinite}
